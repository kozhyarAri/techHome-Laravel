        ...
        <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
            ...
            <tbody>
                @foreach ($emailMessage as $message)
                    <tr class="{{ $message->state == 0 ? 'bg-red-200': 'bg-white' }} border-b">
                        ...
                        <td class="px-6 py-4 flex gap-1">
                            <a class="bg-green-400 px-2 py-1 rounded-md text-white"
                                href="{{ route('emailMessage.show', ['emailMessage' => $message->id]) }}">Open</a>
                            <form method="POST" action="{{ route('emailMessage.destroy', ['emailMessage' => $message->id]) }}">
                                @csrf
                                @method('DELETE')
